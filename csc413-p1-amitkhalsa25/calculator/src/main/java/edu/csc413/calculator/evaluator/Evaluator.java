...
public class Evaluator {
  ...
  //Checking point for closed and open parenthesis operators().
    ...
    while (this.expressionTokenizer.hasMoreTokens()) {
      ...
      if (!(expressionToken = this.expressionTokenizer.nextToken()).equals(" ")) {
        ...
        if (Operand.check(expressionToken)) {
          ...
          //Begin the use of the operators :)
          ...
          if (expressionToken.equals("(")) {
            //pushing the parenthesis then.
            operatorStack.push(newOperator);

          } else if (expressionToken.equals(")")) {
            while (operatorStack.peek().priority() > 0 && !operatorStack.isEmpty()) {
              process();
            }
            //Checks if parenthesis are incomplete
            if (!operatorStack.isEmpty())
              operatorStack.pop();
          } else {
            while (operandStack.size() >= 2 && operatorStack.peek().priority() >= newOperator.priority()) {
              process();
            }
            operatorStack.push(newOperator);
          }
